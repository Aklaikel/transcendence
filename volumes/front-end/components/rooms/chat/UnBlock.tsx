...
export default function UnBlock() {
  ...
  return (
    <div className="py-4 px-4 h-full">
     ...
     {
      !user.userBlockedRoom.length ? <div className='text-center'>no user blocked found</div>:
      user.userBlockedRoom.map((ele: any) => {
        return (
          <div key={ele.id} className="flex justify-between items-center gap-4 py-2">
            ...
            >
           {update ?<Loader2 className=' animate-spin' /> :<DoorOpen /> }   unblock
            </button>
          </div>
        );
      })}
